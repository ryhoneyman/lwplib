   ...
   EVAL: {"eval": "EVALUATION"}
     ...
     OPERATORS:
        (Can be prefixed with {MINMATCH,MAXMATCH} in case of regex detected KEY
           ...
           "??": checks the existence of value (returns boolean)
...
class EvalEngine extends Base
{
      ...
      if (isset($input['return'])) {
         ...
         if (preg_match("/^(\S+)\s+(\S+)\s+((?:['\"])?.*(?:['\"])?)$/",$input['eval'],$match)) {
            ...
            // Single match key found
               ...
               if ($matchOp != '??' && !array_key_exists($matchKey,$values)) {
            ...
            if ($enforceCount && $minMatch == 0 && $maxMatch == $listCount) {
               ...
               foreach ($matchList as $listKey) {
                  $result = $this->operator($listKey,$values[$listKey],$matchOp,$matchValue,array_key_exists($listKey,$values));
   ...
   public function operator($evalKey, $evalValue, $evalOp, $matchValue, $keyExists)
   {
      $this->debug(9,"Evaluating: $evalKey $evalOp ".json_encode(array('exists' => $keyExists, 'match' => $matchValue, 'value' => $evalValue)).")");
      ...
      switch ($evalOp) {
         ...
         case "??": $result = ($keyExists == $matchValue); break;
      }
